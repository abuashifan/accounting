...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <a class="btn btn-sm btn-primary" href="{{ route('debug.purchase-invoices.create') }}">Create Purchase Invoice</a>
    </div>
    ...
    <div class="alert alert-warning small mb-3" id="dangerBox" style="display:none">
        ...
        <div class="mt-1">
            Recommended for posted transactions: <span class="fw-semibold">VOID/REVERSAL</span> and <span class="fw-semibold">RETURN</span>.
    ...
    <script>
        (() => {
                    ...
                    tbody.insertAdjacentHTML('beforeend', `
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                ${isPosted && !isVoided ? `<button class="btn btn-sm btn-outline-warning" type="button" data-void="${inv.id}">Void</button>` : ''}
                                <a class="btn btn-sm btn-outline-secondary ${canDanger ? '' : 'disabled'}" ${canDanger ? `href="{{ url('/debug/purchase-invoices') }}/${inv.id}/edit"` : 'href="#" aria-disabled="true" tabindex="-1"'}>Edit</a>
                                <button class="btn btn-sm btn-outline-danger" type="button" data-delete="${inv.id}" ${canDanger ? '' : 'disabled'}>Delete</button>
                            </td>
                        </tr>
                    `);
                }
            ...
            tbody.addEventListener('click', async (e) => {
                ...
                const r = await window.DebugApi.apiFetch(`/api/purchase-invoices/${id}/post`, { method: 'POST' });
            ...
            tbody.addEventListener('click', async (e) => {
                ...
                if (!confirm(`Void purchase invoice #${id}?`)) return;

                const r = await window.DebugApi.apiFetch(`/api/purchase-invoices/${id}/void`, {
                    method: 'POST',
                ...
                window.DebugApi.showAlert('success', 'Purchase invoice voided');
                load().catch(() => {});
            });
