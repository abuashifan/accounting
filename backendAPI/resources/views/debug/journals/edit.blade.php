...
@section('content')
    <div class="d-flex align-items-center justify-content-between mb-3">
        <h1 class="h4 m-0">Edit Journal #{{ $id }}</h1>
    ...
    <div class="card">
        <div class="card-body">
            <form id="journalForm">
                ...
                <button class="btn btn-primary btn-sm" type="submit">Submit → PUT /api/journals/{{ $id }}</button>
            </form>
        </div>
    </div>
@endsection
    ...
    <script>
        (() => {
            ...
            async function load() {
                ...
                document.getElementById('period').value = journal.accounting_period_id || '';
                document.getElementById('description').value = journal.description || '';

                tbody.innerHTML = '';
                const lines = journal.journal_lines || journal.journalLines || [];
            ...
            document.getElementById('journalForm').addEventListener('submit', async (e) => {
                ...
                const res = await window.DebugApi.apiJson(`/api/journals/${id}`, {
                    method: 'PUT',
                    body: JSON.stringify(payload),
                });

                if (res?.success) {
                    window.DebugApi.showAlert('success', 'Journal updated');
